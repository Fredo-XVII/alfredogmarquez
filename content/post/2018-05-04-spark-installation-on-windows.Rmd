...
sparkR.uiWebUrl()
```
Sometimes you get an IP address, 172.16.0.200:4040, and sometimes you get a blob such as http://LAPTOP-OK0GO7H8:4040.  If you do get a blob, go to your cmd(command line) and type in "ipconfig /all" ([link](https://managewp.com/blog/how-to-access-a-local-website-from-internet-with-port-forwarding)) to get the IP address to your local host.  There you will find the IP4v address, add the :4040 to end and paste into your browser.  In any case, you should see the Spark Application UI (see below).
...
df %>% SparkR::collect() %>%
  ...
  geom_smooth()
```
The plot of faithful data shows a clear distinction between 2 clusters, which is not discernable from the correlation of .90.  
```{r}
...
# Conclusion
If you are interested in learning and using Spark, you should be able to do that on any PC or laptop without the need of an entire Hadoop cluster.  This blog covered the need software and installation instructions to get you up and running.  Finally, I provided a simple example of to show that the SparkR installation is working.  If you would like to learn about Spark and SparkR look up the references below.
