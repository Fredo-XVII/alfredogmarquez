...
#### SPARK_HOME:
The process to set up the environmental variables on your machine will be the similar for Spark as it is for the other variables below, where you provide a variable name, variable value and at it to the Path variable.  So I will use Spark as a example that you can apply to the other variables.

  1. Locate the "bin" file path for the Spark version you downloaded to add to your environmental list.  For Spark, the path is in my "C:\\Apps" folder, or "C:\\Apps\\Apache\\Spark\\spark-2.3.0\\bin", copy the file path using ctrl + c.
...
Like Java, Winutils does not reference the "bin" folder. I will reference the path of the master folder. 
