...
**Load Packages**
I am going to load the tidyverse package to drive a point.  The Spark package masks many functions in the tidyverse universe, so it matters when you call a function. When using SparkR along side other R packages, I recommend using the double colon to call your functions, at least the SparkR functions.  For example, instead of sq(), use SparkR::sql(). 
```{r}
library(tidyverse)
```
...
Now that you have the SparkR package loaded, you can create a spark session on you local machine. There are 4 options below that are mandatory.  
  ...
  - **sparkHome** -  field also took me a long time to figure out. I came across on some blog a while ago, so I can't give them credit, but that was a big win.  Provide the path to your Spark home directory to this field. This may not have to be done on a Hadoop system.
...
All the other settings are optional from what I found, such as the spark.driver.memory because they come with initial defaults. For example, the spark.driver.memory is defaulted to 1g.

**Look at your Spark Session Stats:**

Paste the url provided below in a new browser tab to look at the Spark session running on your machine.  You will able to see a history of the Spark commands you submit on this site.
```{r}
sparkR.uiWebUrl()
```
Sometimes you get an IP address, 172.16.0.113:4040, and sometimes you get a blob such as http://LAPTOP-OK0GO7H8:4040.  If you do get a blob, go to your cmd and type in "ipconfig /all" ([link](https://managewp.com/blog/how-to-access-a-local-website-from-internet-with-port-forwarding)) to get the IP address to your local host.  There you will find the IP4v address, add the :4040 to end and paste into your browser.  In any case, you should see the Spark Application UI (see below).
...
df %>% SparkR::collect() %>%
  ...
  geom_smooth()
```
The plot of faithful data shows a clear distinction between 2 clusters.  
```{r}
